...
function ScoutloopMark({ className = '' }: { className?: string }) {
  return (
    <svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" className={className} role="img" aria-label="Scoutloop">
      <path fillRule="evenodd" clipRule="evenodd" fill="currentColor" d="M50 11 A39 39 0 1 0 50 89 A39 39 0 1 0 50 11 Z M12.3 60 L33 42 L48 58 L62 35 L87.7 60 A39 39 0 0 1 12.3 60 Z" />
    </svg>
  )
}
          ...
          <div className="rounded-2xl overflow-hidden border border-gray-100">
            <div className="grid grid-cols-1 lg:grid-cols-2">
              ...
              <div className="bg-off-white p-8 sm:p-12 space-y-6">
                <div className="flex items-center gap-4">
                  ...
                  <div className="space-y-1">
                    <span className="text-xs font-bold tracking-widest text-glacier uppercase">Built in BC</span>
                    <h2 className="text-3xl sm:text-4xl font-extrabold text-navy tracking-tight leading-tight">Scoutloop</h2>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-3">
                  {[{ name: 'Next.js', icon: 'nextjs' }, { name: 'React', icon: 'react' }, { name: 'Supabase', icon: 'supabase' }, { name: 'Vercel', icon: 'vercel' }, { name: 'Tailwind', icon: 'tailwind' }, { name: 'Python', icon: 'python' }, { name: 'FastAPI', icon: 'fastapi' }].map((tech) => (
                    <div key={tech.name} className="flex items-center gap-1.5 bg-glacier/5 text-glacier rounded-lg px-2.5 py-1.5">
                      <Image src={`/logos/tools/${tech.icon}.svg`} alt={`${tech.name} logo`} className="w-4 h-4 shrink-0" width={16} height={16} loading="lazy" decoding="async" onError={(e) => { const target = e.target as HTMLImageElement; target.style.display = 'none' }} />
                      <span className="text-xs font-semibold">{tech.name}</span>
                    </div>
                  ))}
                </div>
                <a href="https://scoutloop.me" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-glacier font-semibold hover:underline group focus-visible:ring-2 focus-visible:ring-glacier focus-visible:ring-offset-2 rounded focus:outline-none">
                  Visit Scoutloop
              ...
              <div className="bg-navy relative flex items-center justify-center p-12 min-h-[320px]">
                <svg className="absolute inset-0 w-full h-full opacity-10" xmlns="http://www.w3.org/2000/svg" aria-hidden>
                  <defs>
                ...
                <div className="relative z-10 text-center space-y-6">
                  ...
                  <div className="space-y-2">
                    <p className="text-white/90 font-semibold text-lg">Know before you go.</p>
                    <p className="text-white/50 text-sm">scoutloop.me</p>
                  </div>
    <svg
